...
let edxFilterState = {
    ...
    duration: 'all',    // duration range from #duration-pills
...
function populateDynamicFilters() {
    ...
    if (langSet.size === 0) langSet.add('English');

    const langPillsWrap = document.getElementById('language-pills');
    if (langPillsWrap) {
        let langHtml = '<button type="button" class="type-pill active" data-language="all" aria-pressed="true">Any <span class="filter-count">0</span></button>';
        Array.from(langSet).sort().forEach(l => {
            langHtml += `<button type="button" class="type-pill" data-language="${escHtml(l)}" aria-pressed="false">${escHtml(l)} <span class="filter-count">0</span></button>`;
        });

        const currentLang = edxFilterState.language;
        langPillsWrap.innerHTML = langHtml;
        langPillsWrap.querySelectorAll('.type-pill').forEach(btn => {
            const active = (btn.dataset.language || 'all') === (currentLang || 'all');
...
function parseDurationWeeks(course) {
    if (course.duration_weeks) return Number(course.duration_weeks);
    const d = String(course.duration || '').toLowerCase();
    const m = d.match(/(\d+)\s*months?/i);
    if (m) return Number(m[1]) * 4.345;
    const w = d.match(/(\d+)\s*weeks?/i);
    if (w) return Number(w[1]);
    const y = d.match(/(\d+)\s*years?/i);
    if (y) return Number(y[1]) * 52;
    return null;
}

function matchDuration(course, duration) {
    if (!duration || duration === 'all') return true;
    const weeks = parseDurationWeeks(course);
    if (weeks === null) return true; // Unknown durations pass through rather than disappearing
    if (duration === 'short') return weeks < 4;
    if (duration === 'medium') return weeks >= 4 && weeks < 26;
    if (duration === 'long') return weeks >= 26;
    return true;
}

function matchMode(course, mode) {
    if (!mode || mode === 'all') return true;
    const courseMode = String(course.mode || 'Online').toLowerCase();
    if (mode.toLowerCase() === 'online') return courseMode.includes('online') || courseMode === 'self-paced';
    if (mode.toLowerCase() === 'offline') return courseMode.includes('offline') || courseMode.includes('in-person') || courseMode.includes('on-campus') || courseMode.includes('classroom');
    if (mode.toLowerCase() === 'hybrid') return courseMode.includes('hybrid') || courseMode.includes('blended');
    return courseMode.includes(mode.toLowerCase());
}

function matchLanguage(course, language) {
    if (!language || language === 'all') return true;
    return String(course.language || 'English').toLowerCase() === language.toLowerCase();
}

function matchRanking(course, ranking) {
...
function getEdxFilteredBase(exclude = {}) {
    ...
    let result = allCoursesData.filter(c =>
        ...
        matchDuration(c, state.duration) &&
        matchMode(c, state.mode) &&
        matchLanguage(c, state.language) &&
        matchRanking(c, state.ranking)
    );
    if (q) {
...
function updateFilterCounts() {
    ...
    document.querySelectorAll('#duration-pills .type-pill').forEach(b => {
        ...
        durationCounts[duration] = durationBase.filter(c => matchDuration(c, duration)).length;
    });
    document.querySelectorAll('#mode-pills .type-pill').forEach(b => {
        const mode = b.dataset.mode || 'all';
        modeCounts[mode] = modeBase.filter(c => matchMode(c, mode)).length;
    });
    document.querySelectorAll('#language-pills .type-pill').forEach(b => {
        const language = b.dataset.language || 'all';
        languageCounts[language] = languageBase.filter(c => matchLanguage(c, language)).length;
    });
    document.querySelectorAll('#ranking-pills .type-pill').forEach(b => {
        const ranking = b.dataset.ranking || 'all';
        rankingCounts[ranking] = rankingBase.filter(c => matchRanking(c, ranking)).length;
    });

    const setCount = (selector, keyAttr, counts, labelFn) => {
        document.querySelectorAll(selector).forEach(b => {
            const key = b.dataset[keyAttr] || 'all';
            const count = counts[key] ?? 0;
            const countEl = b.querySelector('.filter-count');
            if (countEl) countEl.textContent = count;
            b.title = labelFn(key, count);
        });
    };

    setCount('#course-type-pills .type-pill', 'type', typeCounts, (k, c) => `${k === 'all' ? 'All' : k} courses (${c})`);
    setCount('#access-type-pills .type-pill', 'access', accessCounts, (k, c) => `${k} courses (${c})`);
    setCount('#domain-chips-scroll .domain-chip', 'domain', domainCounts, (k, c) => `${k === 'all' ? 'Featured' : k} (${c})`);
    setCount('#skill-level-pills .type-pill', 'level', levelCounts, (k, c) => `${k === 'all' ? 'All' : k} level (${c})`);
    setCount('#duration-pills .type-pill', 'duration', durationCounts, (k, c) => {
        const labels = { all: 'Any duration', short: 'Under 4 weeks', medium: '1–6 months', long: '6+ months' };
        return `${labels[k] || k} (${c})`;
    });
    setCount('#mode-pills .type-pill', 'mode', modeCounts, (k, c) => `${k === 'all' ? 'Any mode' : k} (${c})`);
    setCount('#language-pills .type-pill', 'language', languageCounts, (k, c) => `${k === 'all' ? 'Any language' : k} (${c})`);
    setCount('#ranking-pills .type-pill', 'ranking', rankingCounts, (k, c) => {
        const labels = { all: 'Any ranking', qs: 'QS Ranked', nirf: 'NIRF Rated' };
        return `${labels[k] || k} (${c})`;
    });
}
...
function updateHeroState(total) {
    ...
    if (f.duration && f.duration !== 'all') parts.push(`${f.duration} duration`);
    if (f.mode && f.mode !== 'all') parts.push(`${f.mode} delivery`);
    if (f.language && f.language !== 'all') parts.push(`${f.language}`);
    if (f.ranking && f.ranking !== 'all') parts.push(`${f.ranking} rated`);
...
function renderActiveFilterChips() {
    ...
    if (f.duration && f.duration !== 'all') {
        const durationLabels = { short: 'Under 4 weeks', medium: '1–6 months', long: '6+ months' };
        chips.push({ key: 'duration', label: durationLabels[f.duration] || f.duration, cls: 'chip-duration' });
    }
    ...
    if (f.ranking && f.ranking !== 'all') {
        const rankingLabels = { qs: 'QS Ranked', nirf: 'NIRF Rated' };
        chips.push({ key: 'ranking', label: rankingLabels[f.ranking] || f.ranking, cls: 'chip-ranking' });
    }
...
function setEdxDuration(duration) {
    edxFilterState.duration = duration || 'all';
    edxFilterState.page = 1;
    document.querySelectorAll('#duration-pills .type-pill').forEach(b => {
        const active = b.dataset.duration === edxFilterState.duration;
        b.classList.toggle('active', active);
        b.setAttribute('aria-pressed', String(active));
    });
    renderEdxCards();
...
function setEdxMode(mode) {
    edxFilterState.mode = mode || 'all';
    edxFilterState.page = 1;
    document.querySelectorAll('#mode-pills .type-pill').forEach(b => {
        const active = b.dataset.mode === edxFilterState.mode;
        b.classList.toggle('active', active);
        b.setAttribute('aria-pressed', String(active));
    });
    renderEdxCards();
...
function setEdxLanguage(language) {
    edxFilterState.language = language || 'all';
    edxFilterState.page = 1;
    document.querySelectorAll('#language-pills .type-pill').forEach(b => {
        const active = b.dataset.language === edxFilterState.language;
        b.classList.toggle('active', active);
        b.setAttribute('aria-pressed', String(active));
    });
    renderEdxCards();
...
function setEdxRanking(ranking) {
    edxFilterState.ranking = ranking || 'all';
    edxFilterState.page = 1;
    document.querySelectorAll('#ranking-pills .type-pill').forEach(b => {
        const active = b.dataset.ranking === edxFilterState.ranking;
        b.classList.toggle('active', active);
        b.setAttribute('aria-pressed', String(active));
    });
    renderEdxCards();
...
function initEdxControls() {
    ...
    if (durationPills) {
        durationPills.addEventListener('click', e => {
            const btn = e.target.closest('.type-pill');
            if (btn) setEdxDuration(btn.dataset.duration || 'all');
        });
    }
    ...
    if (modePills) {
        modePills.addEventListener('click', e => {
            const btn = e.target.closest('.type-pill');
            if (btn) setEdxMode(btn.dataset.mode || 'all');
        });
    }
    ...
    if (languagePills) {
        languagePills.addEventListener('click', e => {
            const btn = e.target.closest('.type-pill');
            if (btn) setEdxLanguage(btn.dataset.language || 'all');
        });
    }
    ...
    if (rankingPills) {
        rankingPills.addEventListener('click', e => {
            const btn = e.target.closest('.type-pill');
            if (btn) setEdxRanking(btn.dataset.ranking || 'all');
        });
    }
...
function initScrollRail() {
    ...
    function updateThumb() {
        ...
        thumbHeight = Math.max(60, Math.round(railHeight * ratio));
        ...
        const top = Math.min(maxTop, Math.round((scrollTop / scrollHeight) * maxTop));
    ...
    function scrollToThumb(top) {
        ...
        const scrollHeight = doc.scrollHeight - doc.clientHeight;
        const maxTop = railHeight - thumbHeight;
        const pct = Math.max(0, Math.min(1, top / maxTop));
        window.scrollTo({ top: Math.round(pct * scrollHeight), behavior: 'auto' });
    }
    ...
    window.addEventListener('mousemove', e => {
        ...
        const currentThumbTop = ((currentTop / Math.max(1, scrollHeight)) * maxTop) + deltaY;
        scrollToThumb(currentThumbTop);
    });
...
function clearAllCourseFilters() {
    edxFilterState = { typePill: 'all', accessTypes: [], domainChip: 'all', levelPill: 'all', country: 'all', duration: 'all', mode: 'all', language: 'all', ranking: 'all', search: '', sort: 'relevance', view: edxFilterState.view, page: 1 };
...
function initOnboarding() {
    ...
    const restartBtn = document.getElementById('onboarding-restart');
    ...
    dots.forEach(dot => {
        dot.addEventListener('click', () => {
            ...
            if (step === 4 && !canShowOnboardingResult()) return;
            renderOnboardingStep(step);
        });
    });

    // Onboarding no longer auto-opens. Users can start it explicitly from the hero CTA or help button.
...
function canShowOnboardingResult() {
    return onboardingState.level && onboardingState.domain && onboardingState.goal;
}

function getMaxOnboardingStep() {
    if (canShowOnboardingResult()) return 4;
    if (onboardingState.goal) return 4;
    if (onboardingState.domain) return 3;
    if (onboardingState.level) return 2;
    return 1;
}
...
function renderOnboardingStep(step) {
    ...
    document.querySelectorAll('.onboarding-dots .dot').forEach(d => {
        const dotStep = parseInt(d.dataset.step, 10);
        const active = dotStep === step;
        const reachable = dotStep <= getMaxOnboardingStep() || (dotStep === 4 && canShowOnboardingResult());
        d.classList.toggle('active', active);
        d.disabled = !reachable;
        d.setAttribute('aria-selected', String(active));
    });

    // Progress bar: step 1=25%, 2=50%, 3=75%, 4=100%
    const progressBar = document.getElementById('onboarding-progress-bar');
    if (progressBar) progressBar.style.width = `${Math.min(100, step * 25)}%`;

    // Restore selected states when returning to previous steps
    document.querySelectorAll('.onboarding-step[data-step="1"] .onboarding-option').forEach(b => {
        b.classList.toggle('selected', b.dataset.level === onboardingState.level);
    });
    document.querySelectorAll('.onboarding-step[data-step="2"] .onboarding-option').forEach(b => {
        b.classList.toggle('selected', b.dataset.domain === onboardingState.domain);
    });
    document.querySelectorAll('.onboarding-step[data-step="3"] .onboarding-option').forEach(b => {
        b.classList.toggle('selected', b.dataset.goal === onboardingState.goal);
    });
    ...
    if (step === 4) {
        const icon = document.querySelector('.onboarding-result-icon');
        ...
        const goalEmoji = { career: '💼', upskill: '🚀', degree: '🎓', cert: '🏅' }[onboardingState.goal] || '🎯';
        ...
        if (icon) icon.textContent = goalEmoji;
        if (title) title.textContent = `${count.toLocaleString()} course${count === 1 ? '' : 's'} matched for you`;
        const rolePhrase = {
        ...
        if (text) text.textContent = `You’re ${rolePhrase} in ${domainPhrase} and aiming for ${goalPhrase}. We filtered the catalog to the courses that best fit your choices — you can still adjust filters anytime.`;
        if (tags) {
            tags.innerHTML = `
